...
function CharacterInfo({ info }) {
  ...
  const getFontColor = (name) => {
    ...
    switch (name) {
      default:
      case '엠버 루멘':
        fontColor = 'text-elementalRed';
        break;
      case '웨이드 리플':
        fontColor = 'text-elementalBlue';
        break;
      case '클로드':
        fontColor = 'text-elementalGreen';
        break;
      case '게일 쿠물러스':
        fontColor = 'text-elementalPurple';
        break;
    }
  ...
  return (
    <dl className="grid gap-7 grid-rows-[0.5fr_auto_1fr] grid-cols-2 max-w-[1280px] ml-8">
      <dt
        className={`row-span-1 col-start-1 col-end-2 self-end text-2xl font-bold animate-fadeInLeftFirst ${getFontColor(
          name
        )}`}
      >
        {name}({nameEn})
      ...
      <dd
        className={`row-span-full col-start-2 col-end-3 justify-self-center animate-fadeIn max-h-1920px`}
      >
        <img
          src={`${getInfoImageURL(characterData, 'characterImage')}`}
          alt={name}
          className="max-h-[40rem]"
        />
      </dd>
      <dd
        className={`row-span-1 col-start-1 col-end-2 text-lg italic animate-fadeInLeftFirst`}
      >
        &#34;{slogan}&#34;
      </dd>
      <dd
        className={`row-span-1 col-start-1 col-end-2 animate-fadeInLeftSecond`}
      >
        {character}
      </dd>
    </dl>
